...
Deno.serve(async (req) => {
  try {
    ...
    // Hash the token
    const tokenNoHash = createHash('sha256').update(token).digest('hex');

    // Find employer email verification by token hash
    const verifications = await base44.asServiceRole.entities.EmployerEmailVerification.filter({ tokenNoHash });
    ...
    await base44.asServiceRole.entities.Verification.update(emailVerif.verificationId, {
      status: 'COMPLETED',
      finalResult: 'NO',
      finalReason: 'EMAIL_NO'
    });

    // Redirect to response recorded page
    return Response.redirect(`${url.origin}/employer-response-recorded`, 302);
  } catch (error) {
    console.error('Verify employment no error:', error);
