...
Report generated by Indexios Resume Verification Platform
  ...
  const handleSaveAllToFolder = async (results, folderId) => {
    ...
    try {
      // For bulk results, we need to create SavedCandidate entries
      // Since bulk results aren't stored as entities yet, we'll need candidate IDs
      // For now, create a temporary mapping or show a message
      const savedCount = results.length;
      alert(`Would save ${savedCount} candidates to folder. This feature requires the candidates to be first saved to your account.`);
    } catch (error) {
