...
import { Shield, History, User, Briefcase, GraduationCap, Sparkles, ArrowLeft, ExternalLink, Lock, Download, Share2 } from 'lucide-react';
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 py-8 md:py-12">
        ...
        {(!user?.subscription_tier || user?.subscription_tier === 'free') && (
          <motion.div
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-gradient-to-r from-amber-900/40 to-yellow-900/40 border-2 border-yellow-400/60 rounded-xl p-4 mb-6 shadow-lg"
          >
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-yellow-200 font-bold text-sm">FREE PLAN - Limited Analysis</p>
                  <p className="text-white/80 text-xs">
                    {user?.scans_used || 0}/3 scans used • Basic feedback only
                  </p>
                </div>
              </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {currentView === 'result' && selectedCandidate && (
            ...
            >
              ...
              {/* Free Tier Alert - Prominent warning for limited analysis */}
              {(!user?.subscription_tier || user?.subscription_tier === 'free') && (
                <motion.div
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1 }}
                  className="bg-gradient-to-br from-yellow-500/20 via-orange-500/20 to-red-500/20 border-2 border-yellow-400 rounded-2xl p-6 mb-6 shadow-2xl"
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <h3 className="text-yellow-300 font-black text-xl">LIMITED FREE ANALYSIS</h3>
                        <span className="bg-yellow-400 text-black text-xs font-bold px-2 py-1 rounded-full">FREE TIER</span>
                      </div>
                      <p className="text-white text-base mb-4 font-semibold">
                        ⚠️ You're viewing basic feedback only. This scan has limited depth and accuracy.
                      </p>
                      <div className="bg-black/30 rounded-lg p-4 mb-4">
                        <p className="text-white/90 text-sm font-semibold mb-3">Upgrade to unlock:</p>
                        <ul className="text-white text-sm space-y-2">
                          <li className="flex items-start gap-2">
                            <span className="text-emerald-400 font-bold">✓</span>
                            <span>Comprehensive fraud detection & risk scoring</span>
                          </li>
                          <li className="flex items-start gap-2">
                            <span className="text-emerald-400 font-bold">✓</span>
                            <span>Deep background verification & credential checks</span>
                          </li>
                          <li className="flex items-start gap-2">
                            <span className="text-emerald-400 font-bold">✓</span>
                            <span>Career trajectory analysis with industry benchmarks</span>
                          </li>
                          <li className="flex items-start gap-2">
                            <span className="text-emerald-400 font-bold">✓</span>
                            <span>Advanced AI insights & detailed recommendations</span>
                          </li>
                        </ul>
                      </div>
                      <Link to={createPageUrl('Pricing')}>
                        <Button className="bg-yellow-400 hover:bg-yellow-300 text-black font-bold text-base px-8 py-6 shadow-xl">
