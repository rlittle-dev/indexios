        ...
        <AnimatePresence mode="wait">
          ...
          {currentView === 'result' && selectedCandidate && (
            ...
            >
              ...
              {/* Free Tier Alert - Always show for free users */}
              {(!user?.subscription_tier || user?.subscription_tier === 'free') && (
                <motion.div
                  initial={{ opacity: 0, y: -10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="bg-gradient-to-r from-yellow-900/30 to-orange-900/30 border-2 border-yellow-500/50 rounded-xl p-5"
                >
                  <div className="flex items-start gap-3">
                    <Lock className="w-6 h-6 text-yellow-400 flex-shrink-0 mt-0.5" />
                    <div className="flex-1">
                      <h3 className="text-yellow-300 font-bold text-lg mb-2">⚠️ Limited Free Analysis</h3>
                      <p className="text-white text-sm mb-3 font-medium">
                        You're viewing a basic scan with limited feedback. Upgrade for full analysis:
                      </p>
                      <ul className="text-white/90 text-sm space-y-1.5 mb-4">
                        <li>✓ Comprehensive fraud detection algorithms</li>
                        <li>✓ Deep background verification checks</li>
                        <li>✓ Career trajectory analysis with benchmarks</li>
                        <li>✓ Advanced risk assessment & insights</li>
                      </ul>
                      <Link to={createPageUrl('Pricing')}>
                        <Button size="sm" className="bg-yellow-400 hover:bg-yellow-500 text-black font-bold shadow-lg">
                          Upgrade to Pro →
                        </Button>
                      </Link>
                    </div>
